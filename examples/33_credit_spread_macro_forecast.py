"""33 — US credit-spread macro forecast (let the SYSTEM pick the drivers; symbolic + neuro-symbolic).

Same premise as the other macro demos (GS10, S&P 500, Bitcoin, inflation): don't hand-pick
features. Give the platform a BROAD, neutral panel of US macro series and let it tell you
which ones explain the investment-grade credit risk premium — with the readable rules to back it.

  1. Upload a bundled monthly macro panel whose target is IG_SPREAD — the Moody's Baa minus Aaa
     corporate-yield spread, a classic investment-grade credit risk premium
     (``data/credit_macro_panel.csv``, all FRED, US-government public domain; the AAA and BAA
     yields are dropped so the model can't trivially reconstruct their difference).
  2. Build a verified platform and train a forecaster on IG_SPREAD with autoregression turned
     OFF (``autoregressive="none"``) — so the model must explain the spread through the macro
     drivers (rates, activity, labour, money, oil) instead of leaning on its own persistence,
     and so the symbolic correction layer has residual structure to add value to.
...
credit-spread widening/tightening and the readable rules over them, not a market-beating signal.

LIVE RESULTS (bundled snapshot): level R^2 ≈ 0.83; the drivers the system kept are led by
oil (MCOILWTICO), retail sales (RSAFS), capacity utilization (TCU) and housing (HOUST).
...
no API key is needed to run this demo.
    ...
    python 33_credit_spread_macro_forecast.py             # build on the bundled snapshot
    python 33_credit_spread_macro_forecast.py --standard  # skip the verified profile
"""
...
def run_credit_forecast(api, args: argparse.Namespace) -> None:
    if not args.dataset.exists():
        print(f"ERROR: {args.dataset} not found.", file=sys.stderr)
        sys.exit(1)
    total = 5

    print_section(1, total, "Creating domain + uploading the bundled credit-spread macro panel")
    ...
    pid = platform["id"]
    # autoregressive="none": forbid the model from leaning on the spread's own persistence, so it
    # must EXPLAIN it through the macro panel (rates, activity, labour, money, oil) rather than
    # soaking up the autocorrelation — and so residual structure is left for the symbolic
    # correction layer to add value to.
    ...
    ar_note = ("drivers only — no spread own-history features"
    ...
    if accepted:
        ...
        print("  No discovered correction rule beat the neural backtest — the macro drivers "
              "already explain the spread, and the layer honestly adds nothing here.")

    print(f"\nDone. Platform {pid}. The platform chose the drivers and showed its working — "
          "explainable fit + readable rules for the credit spread, not a market-beating signal.")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="US credit-spread macro forecast — let the system pick the drivers")
    ...
    parser.add_argument("--autoregressive", choices=["full", "limited", "none"], default="none",
                        help="how much the model may use the spread's own history (default: none "
    ...
    run_demo(run_credit_forecast, args)
